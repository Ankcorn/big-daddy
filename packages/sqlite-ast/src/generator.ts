...
function generateInsert(stmt: InsertStatement): string {
	...
	sql += " VALUES ";
	sql += stmt.values
		.map((row) => `(${row.map(generateExpression).join(", ")})`)
		.join(", ");
...
function generateTableConstraint(constraint: TableConstraint): string {
	switch (constraint.constraint) {
		case "PRIMARY KEY":
			return (
				"PRIMARY KEY (" +
				constraint.columns?.map(generateIdentifier).join(", ") +
				")"
			);
		case "UNIQUE":
			return (
...
function generateAlterTable(stmt: AlterTableStatement): string {
	...
	switch (stmt.action) {
		...
		case "RENAME COLUMN":
			...
			sql +=
				" RENAME COLUMN " +
				generateIdentifier(stmt.oldColumnName) +
				" TO " +
				generateIdentifier(stmt.newName);
			break;
		case "DROP COLUMN":
...
function generateInExpression(expr: InExpression): string {
	...
	if (
		...
		expr.values[0]?.type === "SubqueryExpression"
	) {
