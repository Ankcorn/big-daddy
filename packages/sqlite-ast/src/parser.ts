...
class Parser {
	...
	private expect(tokenValue: string, type?: string): Token {
		...
		if (!token) {
			throw new ParserError(
				`Expected '${tokenValue}' but reached end of input`,
			);
		}
	...
	public parse(): Statement {
		...
		throw new ParserError(
			`Unexpected statement starting with '${token.token}'`,
			token,
		);
	}
	...
	private parseColumnConstraints(
		terminators: string[] = [",", ")"],
	): ColumnConstraint[] {
	...
	private parseDataType(): string {
		...
		if (
			!dataTypeToken ||
			(dataTypeToken.type !== "identifier" && dataTypeToken.type !== "keyword")
		) {
			throw new ParserError(
	...
	private parsePrimary(): Expression {
		...
		if (token.type === "function") {
			...
			const args = this.match(")")
				? []
				: this.parseCommaSeparatedList((): Expression => {
