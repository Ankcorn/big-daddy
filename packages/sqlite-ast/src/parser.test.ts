...
describe("parser", () => {
				...
				whenClauses: [
					{
						...
						// biome-ignore lint/suspicious/noThenProperty: `then` is a valid property name for CASE WHEN clauses in SQL AST
	...
	describe("UPDATE statements", () => {
		...
		it("should parse UPDATE with RETURNING specific columns", () => {
			const sql =
				"UPDATE users SET name = 'John' WHERE id = 1 RETURNING id, name, email;";
			const ast = parse(sql);
