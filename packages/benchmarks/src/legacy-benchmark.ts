				...
				case "/":
					return new Response(
						JSON.stringify({
							...
							endpoints: {
								"/init":
									"POST - Initialize Big Daddy cluster (required first step)",
...
async function handleSetup(env: Env, correlationId: string): Promise<Response> {
	...
	const sql = await env.BIG_DADDY.createConnection("default", {
		nodes: 8,
		correlationId,
	});
...
async function handleInsert(
	...
	const sql = await getSQL(env, correlationId);
	const result =
		await sql`INSERT INTO users (id, email, name, age, country) VALUES (${user.id}, ${user.email}, ${user.name}, ${user.age}, ${user.country})`;
 ...
 */
async function handleSelectById(
	env: Env,
	correlationId: string,
	url: URL,
): Promise<Response> {
 ...
 */
async function handleSelectByEmail(
	env: Env,
	correlationId: string,
	url: URL,
): Promise<Response> {
...
async function handleUpdate(
	...
	const sql = await getSQL(env, correlationId);
	const result =
		await sql`UPDATE users SET age = ${newAge} WHERE id = ${userId}`;
 ...
 */
async function handleMixedWorkload(
	env: Env,
	correlationId: string,
): Promise<Response> {
	const startTime = Date.now();
