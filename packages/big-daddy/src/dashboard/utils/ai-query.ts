...
export async function parseQueryWithAI(query: string, ai: Ai): Promise<string> {
	...
	try {
		...
		logger.debug`Query parsing failed, attempting to use AI ${{ error: parseError instanceof Error ? parseError.message : String(parseError) }}`;
		...
		try {
			...
			logger.debug`Calling Workers AI to generate SQL ${{ model: "@cf/qwen/qwen2.5-coder-32b-instruct" }}`;
			...
			const selectMatch = generatedQuery.match(
				/SELECT\s+['"`](.+)['"`]\s+AS\s+\w+/is,
			);
			if (selectMatch?.[1]) {
			...
			logger.debug`AI generated SQL query ${{ generatedQuery }}`;
			...
			try {
				...
				logger.error`Generated query failed to parse ${{ generatedQuery, error: generatedParseError instanceof Error ? generatedParseError.message : String(generatedParseError) }}`;
				throw new Error(`AI generated an invalid SQL query: ${generatedQuery}`);
			}
		} catch (aiError) {
			logger.error`Failed to generate SQL query with AI ${{ error: aiError instanceof Error ? aiError.message : String(aiError) }}`;
			throw new Error(
				`Could not parse query and AI generation failed: ${aiError instanceof Error ? aiError.message : String(aiError)}`,
			);
		}
	}
}
