...
describe("parser", () => {
    describe("SELECT statements", () => {
        ...
        it("should parse SELECT with JOIN", () => {
            const sql = "SELECT users.name, orders.total FROM users JOIN orders ON users.id = orders.user_id;";
            const ast = parse(sql);
            expect(ast).toEqual({
                type: "SelectStatement",
                select: [
        ...
        it("should parse SELECT with ORDER BY", () => {
            const sql = "SELECT name, created_at FROM users ORDER BY created_at DESC, name ASC;";
            const ast = parse(sql);
            expect(ast).toEqual({
                type: "SelectStatement",
                select: [
        ...
        it("should parse SELECT with GROUP BY and HAVING", () => {
            const sql = "SELECT category, COUNT(*) FROM products GROUP BY category HAVING COUNT(*) > 5;";
            const ast = parse(sql);
            expect(ast).toEqual({
                type: "SelectStatement",
                select: [
        ...
        it("should parse CASE expression", () => {
            const sql = "SELECT CASE WHEN age < 18 THEN 'minor' ELSE 'adult' END FROM users;";
            const ast = parse(sql);
        ...
        it("should parse parenthesized expressions", () => {
            const sql = "SELECT * FROM users WHERE (age > 18 AND country = 'US') OR admin = 1;";
            const ast = parse(sql);
            expect(ast.where).toEqual({
                type: "BinaryExpression",
                operator: "OR",
    ...
    describe("CREATE TABLE statements", () => {
        it("should parse CREATE TABLE with column definitions", () => {
            const sql = "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT NOT NULL, email TEXT);";
            const ast = parse(sql);
            expect(ast).toEqual({
    ...
    describe("PRAGMA statements", () => {
        ...
        it("should parse PRAGMA with function call (identifier and number arguments)", () => {
            ...
            expect(ast.arguments[0]).toEqual({
                type: "Identifier",
                name: "users",
            });
            expect(ast.arguments[1]).toEqual({
                type: "Literal",
                value: 10,
                raw: "10",
            });
