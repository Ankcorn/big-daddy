...
function generateSelectClause(clause) {
...
function generateInsert(stmt) {
    let sql = "INSERT INTO " + generateIdentifier(stmt.table);
    if (stmt.columns && stmt.columns.length > 0) {
        sql += " (" + stmt.columns.map(generateIdentifier).join(", ") + ")";
    }
    sql += " VALUES ";
    sql += stmt.values.map(row => "(" + row.map(generateExpression).join(", ") + ")").join(", ");
    return sql;
}
function generateUpdate(stmt) {
    let sql = "UPDATE " + generateIdentifier(stmt.table);
    sql += " SET " + stmt.set.map(s => generateIdentifier(s.column) + " = " + generateExpression(s.value)).join(", ");
    if (stmt.where) {
        sql += " WHERE " + generateExpression(stmt.where);
    }
...
function generateCreateTable(stmt) {
...
function generateAlterTable(stmt) {
...
function generateCreateIndex(stmt) {
    let sql = "CREATE";
    if (stmt.unique) {
        sql += " UNIQUE";
    }
...
function generateDropTable(stmt) {
    let sql = "DROP TABLE";
    if (stmt.ifExists) {
        sql += " IF EXISTS";
    }
...
function generateColumnConstraint(constraint) {
...
function generateLiteral(lit) {
    return lit.raw;
}
function generateBinaryExpression(expr) {
    const left = generateExpression(expr.left);
    const right = generateExpression(expr.right);
    return `${left} ${expr.operator} ${right}`;
}
function generateUnaryExpression(expr) {
...
function generateFunctionCall(expr) {
    const args = expr.arguments.map(generateExpression).join(", ");
    return `${expr.name}(${args})`;
}
function generateInExpression(expr) {
...
function generateBetweenExpression(expr) {
    const value = generateExpression(expr.expression);
...
function generateSubqueryExpression(expr) {
    return "(" + generateSelect(expr.query) + ")";
}
function generateCaseExpression(expr) {
    let sql = "CASE";
    if (expr.expression) {
